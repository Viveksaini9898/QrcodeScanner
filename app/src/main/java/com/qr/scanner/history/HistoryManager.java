...
import com.qr.scanner.PreferencesActivity;
import com.qr.scanner.history.SQLite.ORM.InterfaceORM;
import com.qr.scanner.history.historyinterface.HistoryInterface;
...
public class HistoryManager implements HistoryInterface<HistoryItem> {

    private static String TAG = HistoryManager.class.getSimpleName();
    ...
    private static String[] COUNT_COLUMN = {"COUNT(1)"};

    private static String[] ID_COL_PROJECTION = {DBHelper.ID_COL};
    private static String[] ID_DETAIL_COL_PROJECTION = {DBHelper.ID_COL, DBHelper.DETAILS_COL};
    private static Pattern DOUBLE_QUOTE = Pattern.compile("\"", Pattern.LITERAL);
    ...
    @Override
    public void clearAll(@Nullable Context context) {
    ...
    @Nullable
    @Override
    public List<HistoryItem> getAll(@Nullable Context context) {
        SQLiteOpenHelper helper = new DBHelper(context);
        List<HistoryItem> items = new ArrayList<>();
        try (SQLiteDatabase db = helper.getReadableDatabase();
             Cursor cursor = db.query(DBHelper.TABLE_NAME,
    ...
    @Override
    public void add(@Nullable Context context, Result result) {
        /*if (!activity.getIntent().getBooleanExtra(Intents.Scan.SAVE_HISTORY, true) ||
                handler.areContentsSecure() || !enableHistory) {
            return;
        }*/
    ...
    @Override
    public void delete(@Nullable Context context,int number) {
        SQLiteOpenHelper helper = new DBHelper(context);
        try (SQLiteDatabase db = helper.getWritableDatabase();
             Cursor cursor = db.query(DBHelper.TABLE_NAME,
                     ID_COL_PROJECTION,
