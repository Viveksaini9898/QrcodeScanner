...
class QRGEncoder {
    ...
    private fun encodeQRCodeContents(data: String?, bundle: Bundle?, type: String) {
        var data = data
        when (type) {
            QRGContents.Type.TEXT -> if (data != null && data.isNotEmpty()) {
                contents = data
                displayContents = data
                title = "Text"
            }
            ...
            QRGContents.Type.CONTACT -> if (bundle != null) {
                ...
                run {
                    ...
                    while (x < QRGContents.PHONE_KEYS.size) {
                        val phone =
                            trim(bundle.getString(QRGContents.PHONE_KEYS[x]))
                        if (phone != null) {
            ...
            QRGContents.Type.LOCATION -> if (bundle != null) {
                ...
                val latitude: Float = bundle.getFloat("LAT", Float.MAX_VALUE)
                val longitude: Float = bundle.getFloat("LONG", Float.MAX_VALUE)
